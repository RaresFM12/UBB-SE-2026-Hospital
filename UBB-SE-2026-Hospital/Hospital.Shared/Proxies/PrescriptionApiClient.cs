...
public class PrescriptionApiClient : ApiClientBase, IPrescriptionApiClient, IPrescriptionService
{
    ...
    public PrescriptionApiClient(HttpClient httpClient) : base(httpClient)
    {
    ...
    // IPrescriptionService methods
    public Dictionary<int, int> GetItemsFromPrescription(string prescriptionId, Dictionary<int, float> userDiscounts)
        => Task.Run(async () => await PostAsync<object, Dictionary<int, int>>($"{BaseUri}/{prescriptionId}/items", new { userDiscounts }) ?? new Dictionary<int, int>()).GetAwaiter().GetResult();

    public Dictionary<int, int> GetCheapestPrescriptionItems(string prescriptionName, int requiredPills)
        => Task.Run(async () => await GetAsync<Dictionary<int, int>>($"{BaseUri}/cheapest?name={Uri.EscapeDataString(prescriptionName)}&requiredPills={requiredPills}") ?? new Dictionary<int, int>()).GetAwaiter().GetResult();

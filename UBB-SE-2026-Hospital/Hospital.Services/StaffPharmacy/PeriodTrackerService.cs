...
namespace Hospital.Services.StaffPharmacy
{
    public class PeriodTrackerService : IPeriodTrackerService
    {
        ...
        public Hospital.Shared.Models.StaffPharmacy.PeriodTrackerState GetTrackerState(int userId)
        {
            var user = this.usersRepository.GetUserByIdAsync(userId).GetAwaiter().GetResult();
            bool hasTracker = user != null && user.CycleDays > 0;

            return new Hospital.Shared.Models.StaffPharmacy.PeriodTrackerState
            { 
